...
class NGramModel:
    ...
    def compute_conditional_probabilities(self, text_tokens):
        """Compute conditional probabilities on pre-tokenized text counting ngrams.
        ...
        for i in range(0, len(text_tokens)-self.n+1):
            ...
            next_world = text_tokens[i+self.n-1]
            if n_minus_1_gram not in counts:
                counts[n_minus_1_gram] = {}
            counts[n_minus_1_gram][next_world] = counts[n_minus_1_gram].get(next_world, 0) + 1
            
        for n_minus_1_gram, possible_next_worlds in counts.items():
            counts_sum = sum(list(possible_next_worlds.values()))
            for next_world in possible_next_worlds:
                counts[n_minus_1_gram][next_world] /= counts_sum
    ...
    def fit(self, text_tokens, verbose=True):
        ...
        self.compute_conditional_probabilities(text_tokens)
