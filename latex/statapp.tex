...
\fancyhead[L]{}
\fancyhead[R]{ENSAE}
\fancyhead[C]{Modèles de langues neuronaux}
\fancyfoot[C]{\thepage}
\fancyfoot[R]{}
...
En section 2, nous précisons le modèle du Transformer sur lequel se concentre notre étude. Nous donnons une vue globale de son architecture avant de décrire précisément ses composantes.
...
\begin{minipage}[t]{0.55\textwidth}
  \ding{113} Exemple de vocabulaire :
  \begin{itemize}
...
% La formule est vraie seulement pour i>1 donc je mettrais (sous réserve de corriger l'affichage des si i=1 et sinon) :
%$$
%\left\{
%    \begin{array}{ll}
%        \P(T) = \prod_{i=1}^{L+1} \P(X_i = s_i) si i=1 \\
        % \[ \P(X_i=s_i \vert X_1=s_1,\ldots, X_{i-1}=s_{i-1}) = \P(X_i=s_i \vert X_{i-(n-1)}=s_{i-(n-1)},\ldots, X_{i-1}=s_{i-1}) \] sinon
%     \end{array}
% \right.
% $$
...
% Je trouve la formulation P(X_k = s_k) = 0 un peu ambigüe, je propose qqch du genre :
% Etant donné que nous travaillons sur un corpus fini, nous avons une combinaison de mots finis. Il est possible qu'un mot "s" n'apparaisse pas dans le modèle. Sa probabilité d'apparition est donc nulle : $\P(X_k = s) = 0 \forall k $.
...
Pour pallier ce problème et pouvoir généraliser à des séquences de mots non connues, nous pouvons effectuer un \og lissage \fg{}, qui consiste à attribuer une probabilité non nulle pour les mots n'apparaissant jamais dans le corpus.
...
Afin de l'utiliser comme estimateur de la probabilité conditionnelle d'un symbole sachant les précédents (i.e. pour en faire un modèle de langue), il faut représenter l'ensemble des symboles précédents comme un vecteur de $\R^p$ et les probabilités conditionnelles comme un vecteur de $\R^q$, où $(p,q) \in \N^*\times \N^*$.
...
% Si on parle de \widehat{P} je préciserais "modèle conditionnel estimé". Je ne comprends pas la deuxième partie de la phrase "on peut s'intéresser à la génération à l'aide du modèle d'un échantillon de textes plausibles"
...
% On multiplie "le vecteur de probabilités conditionnelles du symbole suivant" par P_j qui est aussi une Proba conditionnelle ? Mauvaise définition de P_j ?
...
% La phrase du dessus me semble être un c/c de la partie précédente non adapté, ne serait-ce pas : $s_i = \arg\max({P_i | i \in \{1,...,k\}})$ ?
...
% Dans la pratique nous pouvons utiliser la notion de température (terme inspiré de la thermodynamique) pour jouer sur les probabilités de sélection. Des températures plus basses rendent le modèle de plus en plus confiant dans ses premiers choix (une température de 0 équivaut à une probabilité argmax) tandis que des températures supérieures à 1 diminuent sa confiance (une température infinie correspond à un échantillonnage uniforme).
...
Voici un schéma de l'allure globale du Transformer telle que présentée pour la première fois dans \cite{vaswani2017attention} pour la tâche de traduction :
\begin{figure}[h]
  \begin{center}
...
\subsection{Mesure de performance : la Perplexité}
...
Le tableau suivant récapitule les performances des modèles que nous avons entraînés (perplexité) :
